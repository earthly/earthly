...
func (wdr *withDockerRun) load(ctx context.Context, opt DockerLoadOpt) error {
	...
	mts, err := wdr.c.buildTarget(ctx, depTarget.String(), opt.Platform, opt.BuildArgs, nil, false, false)
	if err != nil {
		return err
	}
	if opt.ImageName == "" {
...
func (wdr *withDockerRun) solveImage(ctx context.Context, mts *states.MultiTarget, opName string, dockerTag string, opts ...llb.RunOption) error {
	...
	tarContext, err := wdr.c.opt.SolveCache.Add(solveID, func(_ states.StateKey) (llb.State, error) {
