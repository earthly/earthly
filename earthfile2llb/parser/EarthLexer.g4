...
BUILD: 'BUILD' -> pushMode(COMMAND_ARGS_ATOMS_ONLY);
WORKDIR: 'WORKDIR' -> pushMode(COMMAND_ARGS_ATOMS_ONLY);
ENTRYPOINT: 'ENTRYPOINT' -> pushMode(COMMAND_ARGS_ATOMS_ONLY);
GIT_CLONE: 'GIT CLONE' -> pushMode(COMMAND_ARGS_ATOMS_ONLY);
DOCKER_LOAD:
	'DOCKER LOAD' -> pushMode(COMMAND_ARGS_ATOMS_ONLY);
DOCKER_PULL:
	'DOCKER PULL' -> pushMode(COMMAND_ARGS_ATOMS_ONLY);
...
BUILD_R:
	BUILD -> type(BUILD), pushMode(COMMAND_ARGS_ATOMS_ONLY);
WORKDIR_R:
	WORKDIR -> type(WORKDIR), pushMode(COMMAND_ARGS_ATOMS_ONLY);
ENTRYPOINT_R:
	ENTRYPOINT -> type(ENTRYPOINT), pushMode(COMMAND_ARGS_ATOMS_ONLY);
GIT_CLONE_R:
	GIT_CLONE -> type(GIT_CLONE), pushMode(COMMAND_ARGS_ATOMS_ONLY);
DOCKER_LOAD_R:
	DOCKER_LOAD -> type(DOCKER_LOAD), pushMode(COMMAND_ARGS_ATOMS_ONLY);
DOCKER_PULL_R:
	DOCKER_PULL -> type(DOCKER_PULL), pushMode(COMMAND_ARGS_ATOMS_ONLY);
