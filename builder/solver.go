...
func (s *solver) solveDockerWithRegistry(ctx context.Context, state pllb.State, platform specs.Platform, img *image.Image, dockerTag string, outFile string, printOutput bool) error {
	...
	solveOpt := s.newSolveOptDockerWithRegistry(img, dockerTag, onPull)
	noCache := true
	platr := platutil.NewResolver(platform)

	buildFun := func(childCtx context.Context, gwClient gwclient.Client) (*gwclient.Result, error) {
		res := gwclient.NewResult()
		ref, err := llbutil.StateToRef(ctx, gwClient, state, noCache, platr, s.cacheImports.AsMap())
		if err != nil {
		...
		// TODO: probably need to capture the key for retrieval
	...
	eg.Go(func() error {
		...
		_, err := s.bkClient.Build(ctx, *solveOpt, "", buildFun, statusChan)
		if err != nil {
			fmt.Println("client build error", err)
			buildErr = err
			return err
		}
		return nil
...
func (s *solver) newSolveOptDockerWithRegistry(img *image.Image, dockerTag string, onPull pullping.PullCallback) *client.SolveOpt {
