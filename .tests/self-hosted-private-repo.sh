...
mkdir -p ~/.ssh
ssh-keyscan -p $SSH_PORT -H localhost >> ~/.ssh/known_hosts
ssh-keyscan -p $SSH_PORT -H 127.0.0.1 >> ~/.ssh/known_hosts
...
git:
  myserver:
    pattern: 'myserver/([^/]+)'
    substitute: 'ssh://root@$ip:2222/root/my/really/weird/path/\$1.git'
    auth: ssh
EOF
