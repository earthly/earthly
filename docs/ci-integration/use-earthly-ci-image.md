...
```bash
docker run --privileged --rm -v "$PWD":/workspace earthly/earthly:v0.6.28 +my-target
```

Or, if you would like to use an alternative directory:

```bash
docker run --privileged --rm -v "$PWD":/my-dir -w /my-dir earthly/earthly:v0.6.28 +my-target
```
