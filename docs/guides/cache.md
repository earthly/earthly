...
The default cache size is adaptable depending on available space on your system. If you would like to limit the cache size more aggressively, you can specify a different limit by modifying the `cache_size_mb` setting in the [configuration](../earthly-config/earthly-config.md). For example:
