...
Imagine this scenario: You work on a team that owns a service, that, in production runs containerized in a microservice environment.  The totality of the services that make-up production are too numerous reasonably run on a developer machine but thankfully your service only depends on a handful of other services.  You have unit tests but need ways to test your points of integration with your dependencies.  You also want ways to test your service externally, that is you want to exercise it in ways a service consumer might. 
