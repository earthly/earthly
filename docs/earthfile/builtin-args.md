...
| `EARTHLY_TARGET_TAG` | The tag part of the canonical reference of the current target. Note that in some cases, no tag is detected (eg target has no [canonical form](../guides/target-ref.md#canonical-form)), and as such, the value is an empty string | For the example above, the tag would be `john/work` |
