...
| `EARTHLY_GIT_REFS` | The git references of the git commit detected within the build context directory, separated by space. If no git directory is detected, then the value is an empty string. Must be enabled with the `VERSION --git-refs 0.7` feature flag. | `issue-2735-git-ref main` |
